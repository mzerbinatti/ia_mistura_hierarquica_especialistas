...
octave.addpath("C:/Users/miche/Desktop/TRABALHO FINAL/Octave_HME_Arquitetura_Dinamica")
...
class Individuo():
    def __init__(self, max_bits_altura, max_bits_largura, geracao=0):
        self.cromossomo = np.random.randint(2, size=(max_bits_altura, max_bits_largura)) #altura = 2bits / largura=3 bits
        self.max_bits_largura = max_bits_largura
        self.max_bits_altura = max_bits_altura
        self.fitness = -10000
        self.geracao = geracao
        # self.qtde_bits_por_entrada = max_bits_altura * max_bits_largura 
        # self.atualizar()
    ...
    # def atualizar(self):
        # self.x = self.cromossomo[0:self.max_bits_altura] 
        # self.y = self.cromossomo[self.max_bits_altura:self.max_bits_altura+self.max_bits_largura] 
        # self.folhas = self.cromossomo[self.max_bits_largura+self.max_bits_altura::]

    def getProfundidade(self):
        return self.cromossomo[0:self.max_bits_altura] # 0 a 2 (altura) + 1

    def getRamos(self):
        return self.cromossomo[self.max_bits_altura:self.max_bits_largura+self.max_bits_altura] # 2 a 5 (largura)
    
    def getFolhas(self):
        return self.cromossomo[self.max_bits_largura+self.max_bits_altura::] # 2 a 5 (largura)
    
    def atualizarXY(self):
        self.x = self.cromossomo[0:self.max_bits_altura] 
        self.y = self.cromossomo[self.max_bits_altura:self.max_bits_largura+self.max_bits_altura]
        self.folhas = self.cromossomo[self.max_bits_largura+self.max_bits_altura::]
...
class AlgoritmoGenetico():
    ...
    def __init__(self, 
                    ...
                    tipo_crossover=TIPO_CROSSOVER.MATRIZ,
                    taxa_crossover=0.8,
        ...
        self.max_bits_largura = max_bits_largura
        valor_altura = 1
        for i in range(max_bits_altura-1):
            mult = max_bits_largura**(i)
            valor_altura = valor_altura + mult*max_bits_largura
        self.altura_bits_total = valor_altura
        self.largura_bits_total = max_bits_largura
        
        self.bits_folhas = max_bits_largura**(max_bits_altura-1)
        ...
        self.populacao = [] 
        self.melhor_individuo = Individuo(self.altura_bits_total, self.largura_bits_total)
    ...
    def gerar_populacao_inicial(self):
        if len(self.populacao_inicial) <= 0:
            for i in range(self.individuos_por_geracao):
                self.populacao.append(Individuo(self.altura_bits_total, self.largura_bits_total, self.numero_geracao))
        else:
            for i in range(self.individuos_por_geracao):
    ...
    def calcular_funcao(self, individuo, i):
        ...
        self.calcular_arvore(individuo.cromossomo, 0, arquitetura)
        arquitetura = np.array(arquitetura)
        
        print("\nGer" + str(individuo.geracao) + "_Ind" + str(i) + "Arq: " + str(arquitetura))

        chave = str(arquitetura)
        ...
        # else:
        #     octave = Oct2Py()
        #     hme_fit, yexp, z = octave.hmeTest(self.max_bits_altura, self.max_bits_largura, arquitetura, nout=3)
        #     lli = hme_fit.lli
        #     if chave in combinacoes_resultados.keys():
        ...
        hme_fit, yexp, z = octave.hmeTest(self.max_bits_altura, self.max_bits_largura, arquitetura, nout=3)
        lli = hme_fit.lli
        chave = str(arquitetura) + "," + str(lli)
        combinacoes_resultados.append(chave)
        octave.exit()
    ...
    def calcular_arvore_impressao(self, individuo):
        arquitetura = []
        self.calcular_arvore(individuo.cromossomo, 0, arquitetura)
        return arquitetura
    ...
    def imprime_melhor_pior_da_geracao(self):
        ...
        x = self.calcular_arvore_impressao(self.populacao[0])
        x_pior = self.calcular_arvore_impressao(self.populacao[-1])
        
        # if (self.numero_geracao >= self.geracoes):
        print("\nGERAÇÃO %s" % (self.numero_geracao))
        print("Melhor: Fitness: %s Arquitetura: %s " % (melhor_fitness,
                                                            str(x)))
        print("Pior: Fitness: %s Arquitetura: %s" % (pior_fitness,
                                                               str(x_pior)))                                                               
    ...
    def efetuar_crossover(self, pai1, pai2):
        return self.efetuar_crossover_matriz(pai1, pai2)
    ...
    def efetuar_crossover_matriz(self, pai1, pai2):
        mascara = np.random.randint(2, size=self.altura_bits_total)

        filho1 = Individuo(self.altura_bits_total, self.largura_bits_total, self.numero_geracao)
        filho2 = Individuo(self.altura_bits_total, self.largura_bits_total, self.numero_geracao)
    ...
    def efetuar_crossover_duplo_ponto(self, pai1, pai2):
        ...
        if (self.crossover_por_entrada): # Crossover de X com X (0 a 10) e de Y com Y (10 a 20)
            ...
            # filho1 = Individuo(self.altura_bits_total, self.largura_bits_total self.bits_folhas, self.numero_geracao)
            filho1 = Individuo(self.altura_bits_total, self.largura_bits_total, self.numero_geracao)
            xy1 = []
        ...
        if self.tipo_mutacao == TIPO_MUTACAO.TODOS_GENES: # verifica em cada gene e, se a taxa for ok, ajusta o gene
            for filho in novos_filhos:
                for j in range(len(filho.cromossomo)): # iterage entre os 20 bits
                    for c in range(len(filho.cromossomo[j])):
                        # Verifica se efetua a mudança no gene
                        if random.random() < self.taxa_mutacao:
                            if filho.cromossomo[j][c] == 1:
                                filho.cromossomo[j][c] = 0
                            else:
                                filho.cromossomo[j][c] = 1
    ...
    def executar_algoritmo_genetico(self):
        ...
        print("\nMELHOR INDIVIDUO: \nGeração:%s -> \nFitness: %s \nCromossomo: %s \nArquitetura: %s" % (self.melhor_individuo.geracao,
                                                               self.melhor_individuo.fitness,
                                                               self.melhor_individuo.cromossomo,
                                                               self.calcular_arvore_impressao(self.melhor_individuo)))
    ...
    def retornar_descricao_tipo_crossover(self):
        if self.tipo_crossover == TIPO_CROSSOVER.UNICO_PONTO:
            ...
            return "Aritmetico"
        elif self.tipo_crossover == TIPO_CROSSOVER.MATRIZ:
            return "Mascara Matriz"
...
a = AlgoritmoGenetico(  max_bits_altura=3, #2
                        max_bits_largura=3, #3
                        bits_folha_unica=2,
                        geracoes=20, #20,
...
import pickle 
print("\n MELHORES")
with open('C:/Users/miche/Desktop/TRABALHO FINAL/Octave_HME_Arquitetura_Dinamica/Dinamica_Serie1_A_TODOS_Execucoes/melhores_individuos.txt', 'rb') as input:
...
print("\n PIORES")
with open('C:/Users/miche/Desktop/TRABALHO FINAL/Octave_HME_Arquitetura_Dinamica/Dinamica_Serie1_A_TODOS_Execucoes/piores_individuos.txt', 'rb') as input:
    company1 = pickle.load(input)
    for i in company1:
        print(i.fitness)
        print(i.cromossomo)
...
print("\n MEDIAS")
with open('C:/Users/miche/Desktop/TRABALHO FINAL/Octave_HME_Arquitetura_Dinamica/Dinamica_Serie1_A_TODOS_Execucoes/media_fitnesses.txt', 'rb') as input:
    company1 = pickle.load(input)
    for i in company1:
        print(i)

print("\n FITNESSES")
with open('C:/Users/miche/Desktop/TRABALHO FINAL/Octave_HME_Arquitetura_Dinamica/Dinamica_Serie1_A_TODOS_Execucoes/fitnesses.txt', 'rb') as input:
    company1 = pickle.load(input)
    for i in company1:
        print(i)
